        ...
        <div className="mb-6">
          ...
          <div className="mt-3 flex items-center gap-3">
            <div className="flex-1 h-2 bg-otai-border rounded-full overflow-hidden">
              <div
                className="h-full bg-otai-purple rounded-full transition-all"
                style={{ width: `${pct}%` }}
              />
            </div>
                ...
                <div key={mod.id} className="bg-otai-dark border border-otai-border rounded-xl overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      {isExpanded ? (
                        <ChevronDown size={16} className="text-otai-purple" />
                      ) : (
                    ...
                    <div className="border-t border-otai-border divide-y divide-otai-border">
                      {modLessons.map((lesson) => {
                        ...
                        return (
                          ...
                          >
                            {done ? (
                              <CheckCircle2 size={16} className="text-otai-green shrink-0" />
                            ) : (
                              <Circle size={16} className="text-otai-text-muted shrink-0" />
                            )}
                            <div className="flex-1 min-w-0">
                              <span className={`text-sm ${done ? "text-otai-text-muted" : "text-white"}`}>
                                {lesson.title}
                              </span>
                            </div>
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    {course.description && (
                      <p className="text-otai-text-muted text-sm mt-1 line-clamp-2">{course.description}</p>
                    )}
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`text-sm font-bold ${pct === 100 ? "text-otai-green" : "text-otai-purple"}`}>
                      {pct}%
                    </span>
                    <ChevronRight size={16} className="text-otai-text-muted" />
                  </div>
                ...
                <div className="mt-3 h-1.5 bg-otai-border rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${pct === 100 ? "bg-otai-green" : "bg-otai-purple"}`}
                    style={{ width: `${pct}%` }}
                  />
                </div>
              </button>
