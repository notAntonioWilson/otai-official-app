...
export async function GET() {
  try {
    ...
    return NextResponse.json({ clients: clientsRes.data || [], posts: postsRes.data || [], marketers: marketersRes.data || [], assignments: assignmentsRes.data || [] });
  } catch {
...
export async function POST(req: Request) {
  try {
    ...
    if (action === "assign") {
      const { error } = await supabase.from("marketing_client_assignments").insert({ marketer_id: payload.marketer_id, client_id: payload.client_id });
      if (error) return NextResponse.json({ error: error.message }, { status: 400 });
      return NextResponse.json({ success: true });
    }
    if (action === "unassign") {
      const { error } = await supabase.from("marketing_client_assignments").delete().eq("id", payload.id);
      if (error) return NextResponse.json({ error: error.message }, { status: 400 });
      return NextResponse.json({ success: true });
    }
    if (action === "add_post") {
