...
export async function POST(req: NextRequest) {
  ...
  if (action === "add_post") {
    const { error } = await supabase.from("marketing_calendar").insert({ client_id: payload.client_id, marketer_id: user.id, platform: payload.platform, post_type: payload.post_type, scheduled_date: payload.scheduled_date, description: payload.description || null, status: "planned" });
