...
// GET — load marketing data (clients, calendar posts, assignments)
export async function GET(req: NextRequest) {
  ...
  if (!profile || profile.role !== "marketing") {
    return NextResponse.json({ error: "Not a marketing user" }, { status: 403 });
  }
  ...
  if (view === "calendar") {
    ...
    // Also get clients list for the dropdown
    ...
    // Get posts for assigned clients
    ...
    // Also get OTAI internal posts (client_id = 'otai')
    const { data: otaiPosts } = await supabase
      .from("marketing_calendar")
      .select("*")
      .eq("client_id", "otai")
      .order("scheduled_date");
...
// POST — create calendar posts, folders, items
export async function POST(req: NextRequest) {
  ...
  if (action === "create_folder") {
    const { error } = await supabase.from("marketing_content_folders").insert({
      client_id: payload.client_id,
