...
import {
  ...
  Minus, Folder, FileText, ChevronRight, X, Loader2,
  Check, AlertCircle,
} from "lucide-react";
...
export default function MarketingClients() {
  ...
  if (selectedClient) {
    return (
      <div>
        <button onClick={() => setSelectedClient(null)} className="flex items-center gap-2 text-otai-text-secondary hover:text-white mb-4 text-sm transition-colors">
          <ArrowLeft size={14} /> Back to Clients
        </button>
        <h1 className="text-2xl font-bold text-white mb-1">{selectedClient.display_name}</h1>
        {selectedClient.company_name !== selectedClient.display_name && (
          <p className="text-otai-text-secondary text-sm mb-6">{selectedClient.company_name}</p>
        )}
        ...
        <div className="bg-otai-dark border border-otai-border rounded-xl mb-6">
          <div className="p-5 border-b border-otai-border">
            <h2 className="text-white font-semibold text-sm">Social Media Metrics</h2>
          </div>
          {metrics.length === 0 ? (
            <div className="p-8 text-center text-otai-text-muted text-sm">No metrics data yet.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="text-otai-text-muted text-xs uppercase tracking-wide">
                  <th className="p-4 text-left">Platform</th><th className="p-4 text-right">Posts</th>
                  <th className="p-4 text-right">Engagement</th><th className="p-4 text-right">Impressions</th>
                  <th className="p-4 text-right">Reach</th><th className="p-4 text-right">Followers</th>
                </tr></thead>
                <tbody>
                  {metrics.map((m) => {
                    const prev = prevMetrics.find((p) => p.platform === m.platform);
                    return (
                      <tr key={m.id} className="border-t border-otai-border/30">
                        <td className={`p-4 font-medium capitalize ${PLATFORM_COLORS[m.platform] || "text-white"}`}>{m.platform}</td>
        ...
        <div className="bg-otai-dark border border-otai-border rounded-xl">
          <div className="flex items-center justify-between p-5 border-b border-otai-border">
            <h2 className="text-white font-semibold flex items-center gap-2">
              <Folder size={16} className="text-otai-purple" /> Content Folders
            </h2>
            <div className="flex items-center gap-2">
              ...
              {currentFolderId && (
                <button onClick={() => { setShowNewItem(true); setError(""); }}
                  className="flex items-center gap-1 px-3 py-1.5 bg-otai-purple/10 text-otai-purple rounded-lg text-xs hover:bg-otai-purple/20 transition-colors">
                  <FilePlus size={12} /> Add Doc
                </button>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 px-5 py-3 text-xs text-otai-text-muted border-b border-otai-border overflow-x-auto">
            {folderPath.map((p, i) => (
              <span key={i} className="flex items-center gap-1 shrink-0">
                ...
                <button onClick={() => navigateFolder(p.id, p.name)}
                  className={`hover:text-white transition-colors ${i === folderPath.length - 1 ? "text-otai-purple" : ""}`}>{p.name}</button>
              </span>
            ))}
          </div>
          ...
          <div className="p-4">
            {currentFolders.length === 0 && currentItems.length === 0 ? (
              <div className="text-center text-otai-text-muted text-sm py-8">
                {currentFolderId ? "Empty folder. Add docs or subfolders." : "No folders yet. Create one to organize your work."}
              </div>
            ) : (
              <div className="space-y-1">
                ...
                {currentItems.map((item) => (
                  <div key={item.id} className="flex items-start gap-3 px-3 py-2.5 rounded-lg hover:bg-white/[0.03] transition-colors">
                    <FileText size={16} className="text-otai-text-muted shrink-0 mt-0.5" />
                    <div>
                      <p className="text-white text-sm">{item.title}</p>
                      {item.content && <p className="text-otai-text-muted text-xs mt-0.5 line-clamp-2">{item.content}</p>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {showNewItem && (
          <div className="mt-3 bg-otai-dark border border-otai-purple/30 rounded-xl p-4 space-y-3">
            <input type="text" value={newItemForm.title} onChange={(e) => setNewItemForm({ ...newItemForm, title: e.target.value })}
              placeholder="Doc title" autoFocus
            ...
            <textarea value={newItemForm.content} onChange={(e) => setNewItemForm({ ...newItemForm, content: e.target.value })}
              placeholder="Notes / content (optional)" rows={3}
            ...
            <div className="flex items-center gap-3">
              <button onClick={handleCreateItem} disabled={saving}
                className="px-4 py-2 bg-otai-purple text-white rounded-lg text-sm disabled:opacity-50">
                {saving ? <Loader2 size={14} className="animate-spin" /> : "Add Doc"}
              </button>
    ...
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <Users size={24} className="text-otai-purple" /> Clients
        </h1>
