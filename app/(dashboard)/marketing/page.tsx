...
export default function MarketingCalendar() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Build client list with OTAI as first option
      ...
      const allClients: ClientOption[] = [
        { id: "otai", company_name: "OTAI Systems", display_name: "OTAI Systems" },
        ...rawClients,
      ];
  ...
  const goPrevWeek = () => { const d = new Date(weekRef); d.setDate(d.getDate() - 7); setWeekRef(d); };
  const goNextWeek = () => { const d = new Date(weekRef); d.setDate(d.getDate() + 7); setWeekRef(d); };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <Calendar size={24} className="text-otai-purple" /> Content Calendar
          </h1>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <button onClick={goPrevWeek} className="p-2 rounded-lg bg-otai-dark border border-otai-border text-otai-text-secondary hover:text-white transition-colors"><ChevronLeft size={16} /></button>
          <button onClick={goNextWeek} className="p-2 rounded-lg bg-otai-dark border border-otai-border text-otai-text-secondary hover:text-white transition-colors"><ChevronRight size={16} /></button>
          <button onClick={goThisWeek} className="px-3 py-2 rounded-lg bg-otai-dark border border-otai-border text-otai-text-secondary hover:text-white text-xs transition-colors">This Week</button>
        </div>
          ...
          return (
            <div key={dateStr} className={`bg-otai-dark border rounded-xl min-h-[140px] flex flex-col ${isToday ? "border-otai-purple/50" : "border-otai-border"}`}>
              <div className="flex items-center justify-between px-2.5 py-2 border-b border-otai-border">
                ...
                <button onClick={() => openAdd(dateStr)} className="p-0.5 rounded text-otai-text-muted hover:text-otai-purple transition-colors"><Plus size={12} /></button>
              </div>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="relative bg-otai-dark border border-otai-border rounded-2xl w-full max-w-md p-6 z-10">
                  ...
                  <select value={form.client_id} onChange={(e) => setForm({ ...form, client_id: e.target.value })}
                    ...
                    {clients.map((c) => (
                      <option key={c.id} value={c.id}>
                        {c.id === "otai" ? "⚡ OTAI Systems (Internal)" : `${c.display_name}${c.display_name !== c.company_name ? ` (${c.company_name})` : ""}`}
                      </option>
                    ))}
                  </select>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {PLATFORMS.map((p) => (
                    <button key={p.value} type="button" onClick={() => setForm({ ...form, platform: p.value })}
                      className={`px-3 py-2 rounded-lg text-xs border transition-colors ${form.platform === p.value ? p.color + " font-medium" : "border-otai-border text-otai-text-secondary hover:border-otai-purple/40"}`}>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {POST_TYPES.map((t) => (
                    <button key={t} type="button" onClick={() => setForm({ ...form, post_type: t })}
                      className={`px-3 py-2 rounded-lg text-xs border transition-colors ${form.post_type === t ? "border-otai-purple bg-otai-purple/10 text-otai-purple font-medium" : "border-otai-border text-otai-text-secondary hover:border-otai-purple/40"}`}>{t}</button>
                  ))}
                </div>
              </div>
              <div>
            ...
            <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-otai-border">
              <button onClick={() => setShowModal(false)} className="px-4 py-2 text-sm text-otai-text-secondary hover:text-white transition-colors">Cancel</button>
